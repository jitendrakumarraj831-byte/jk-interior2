...
function detectIntent(text: string, ctx: ConversationContext): Intent {
  ...
  // View Catalog
  if (
    /\b(catalog|catalogue|design\s*(?:dekho|dikhao|bhejo|send|share)|brochure|pdf|designs?\s*(?:list|collection|photos?)|kaunse\s*design|design\s*kaunse|latest\s*design|new\s*design|design\s*dekhna|dekhna\s*chahta|dekhni\s*hai)\b/.test(t)
  ) {
    return "view_catalog"
  }
  ...
  // Book Site Visit
  if (
    /\b(site\s*visit|ghar\s*(?:pe|par|aa|aao|bulao)|free\s*(?:visit|measurement|maap)|measurement|naaap|naap\s*lena|naaap\s*lena|visit\s*book|book\s*(?:a\s*)?visit|aao\s*ghar|ghar\s*aakar|visit\s*fix|visit\s*schedule|aana\s*hai|aajao|aa\s*jao|team\s*bhejo)\b/.test(t)
  ) {
    return "book_visit"
  }
...
function extractRoomDimensions(text: string): string | null {
  ...
  const multiCheckRegex = /(\d{1,3})\s*[x×*]\s*(\d{1,3})/gi;
  let matchCount = 0;
  while (multiCheckRegex.exec(text) !== null) {
    matchCount++;
    if (matchCount > 1) {
      // अगर 1 से ज़्यादा रूम के साइज मिले, तो तुरंत फ्लैग रिटर्न कर दें
...
function tryExtractPhone(text: string): string | null {
  // Match Indian mobile numbers: optional +91/0, then 10 digits starting with 6-9
  const match = text.match(/(?:\+91|0)?([6-9]\d{9})\b/)
  return match ? match[1] : null
}

// ─────────────────────────────────────────────────────────────
// [L3] WhatsApp / Webhook Lead Alert (non-blocking)
// ─────────────────────────────────────────────────────────────
...
function sendWhatsAppAlert(
  ...
  if (!webhookUrl) {
    console.warn("[lead-alert] LEAD_WEBHOOK_URL not set — skipping notification")
    return
  }

  const label = name ? `*${name}*` : "एक नया ग्राहक"
  const city = details.city || "N/A"
  const service = details.service || "N/A"
  const source = details.source || "jkinterior.online"
  const intent = details.intent || "N/A"
  const roomSize = details.roomSize || "N/A"

  const text =
    `🔥 *नया HOT LEAD — JK Interior*\n\n` +
    `👤 नाम: ${label}\n` +
    `📱 WhatsApp: +91${phone}\n` +
    `📍 शहर: ${city}\n` +
    `🛠 सर्विस: ${service}\n` +
    ...
    `🎯 Intent: ${intent}\n` +
    `🌐 Source: ${source}\n\n` +
    `⚡ तुरंत follow-up करें!`

  // Fire-and-forget — never await, never block the response
  fetch(webhookUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text, phone, name, city, service, intent, source }),
  })
    .then((r) => {
      if (!r.ok) console.warn(`[lead-alert] Webhook responded ${r.status}`)
      else console.log(`[lead-alert] Alert sent for phone=${phone}`)
    })
    .catch((e) => console.error("[lead-alert] Webhook error:", e))
}
  ...
  try {
    ...
    // ── [L3] Phone extraction + non-blocking lead alert ──────
    const extractedPhone = tryExtractPhone(normMessage) ?? ctx.phone ?? leadContext?.phone
    if (extractedPhone && !ctx.phone) {
      // Store it so subsequent logic can use it
      ctx.phone = extractedPhone
    }
    if (extractedPhone) {
      // Fire-and-forget: notify the business owner immediately
      sendWhatsAppAlert(extractedPhone, ctx.name ?? leadContext?.name, {
        city: ctx.city ?? leadContext?.city,
        service: ctx.service ?? leadContext?.service,
        roomSize: ctx.roomSize ?? leadContext?.roomSize,
        intent,
        source: "jkinterior.online",
      })
    }
    ...
    // ── [NEW] view_catalog intent — instant PDF catalog reply ──
    if (intent === "view_catalog") {
      const catalogReply =
        "जी बिल्कुल! आप नीचे दिए गए लिंक पर क्लिक करके हमारा लेटेस्ट JK Interior Luxury Design Catalog (PDF) डाउनलोड कर सकते हैं: \n\n 📂 [यहाँ क्लिक करके कैटलॉग डाउनलोड करें](https://jkinterior.online/catalog.pdf) \n\nइसमें आपको PVC फॉल्स सीलिंग, वॉल पैनलिंग और लाइट्स के बेहतरीन डिज़ाइन मिल जायेंगे। क्या आप इनमें से कोई डिज़ाइन अपने घर के लिए फाइनल करना चाहते हैं?"
      return ok(catalogReply, "local", { ...buildUpdatedContext(), lastIntent: "view_catalog" })
    }

    // ── [NEW] book_visit intent — capture lead details for site visit ──
    if (intent === "book_visit") {
      const bookVisitReply =
        "जी जरूर! हमारी टीम आपके साइट पर आकर बिल्कुल फ्री में सही नाप (Measurement) लेगी और आपको डिज़ाइन कैटलॉग भी दिखाएगी। इसके लिए कृपया अपना **पूरा नाम, शहर/लोकेशन और व्हाट्सएप नंबर** यहाँ टाइप करें ताकि हम विज़िट कन्फर्म कर सकें। 😊"
      return ok(bookVisitReply, "local", { ...buildUpdatedContext(), lastIntent: "book_visit", conversationStage: "booking" })
    }
