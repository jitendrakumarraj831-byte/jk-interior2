...
function CardThumb({ img, index, onOpen }: { img: GalleryImage; index: number; onOpen: (img: GalleryImage) => void }) {
  return (
    <motion.div
      ...
      className="group relative aspect-square overflow-hidden rounded-xl cursor-pointer border border-gray-200 hover:border-emerald-300 transition-all duration-500 hover:shadow-[0_8px_30px_rgba(5,150,105,0.15)]"
    >
      <img src={img.src} alt={img.alt} className="object-cover transition-transform duration-700 group-hover:scale-110 w-full h-full" loading="lazy" />
...
function FeaturedSection({ images, onOpen }: { images: GalleryImage[]; sectionIndex: number; onOpen: (img: GalleryImage) => void }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
      ...
      >
        <img src={featured.src} alt={featured.alt} className="object-cover transition-transform duration-700 group-hover:scale-105 w-full h-full" loading="lazy" />
    ...
    <div className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-luxury md:grid md:grid-cols-4 md:overflow-visible">
      {images.map((img, i) => (
        <motion.div
          ...
          onKeyDown={(e) => e.key === "Enter" && onOpen(img)}
          aria-label={`View: ${img.alt}`}
          className="group relative flex-shrink-0 w-[68vw] md:w-auto snap-start aspect-[3/4] overflow-hidden rounded-2xl cursor-pointer border border-gray-200 hover:border-emerald-300 transition-all duration-500 hover:shadow-lg"
        >
          <img src={img.src} alt={img.alt} className="object-cover transition-transform duration-700 group-hover:scale-110 w-full h-full" loading="lazy" />
    ...
    <div className="columns-2 md:columns-3 gap-3 md:gap-4 [column-fill:_balance]">
      {images.map((img, i) => (
        <motion.div
          ...
          onKeyDown={(e) => e.key === "Enter" && onOpen(img)}
          aria-label={`View: ${img.alt}`}
          className="group relative block w-full overflow-hidden rounded-xl cursor-pointer border border-gray-200 hover:border-emerald-300 mb-3 md:mb-4 break-inside-avoid transition-all duration-500 hover:shadow-lg"
        >
          <img src={img.src} alt={img.alt} className="object-cover transition-transform duration-700 group-hover:scale-110 w-full h-full" loading="lazy" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-6 lg:px-12 pt-20 sm:pt-24 pb-12">
        ...
        >
          ...
          <p className="text-gray-600 max-w-2xl mx-auto text-base md:text-lg leading-relaxed mb-8">
            Budget आपका, ज़िम्मेदारी हमारी! पाइए <span className="text-amber-500 font-semibold">Premium Interior</span> और <span className="text-emerald-600 font-semibold">False Ceiling</span> solutions!
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <a
              href="tel:+918651070831"
              aria-label="Call JK Interior"
              className="flex items-center justify-center gap-2 px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold rounded-xl transition-all shadow-[0_4px_20px_rgba(5,150,105,0.3)] active:scale-95"
            >
              <Phone size={18} aria-hidden="true" /> अभी कॉल करें
            </a>
            <a
              href="/contact"
              className="flex items-center justify-center gap-2 px-8 py-4 border border-emerald-300 bg-emerald-50 hover:border-emerald-400 hover:bg-emerald-100 text-emerald-700 text-sm font-bold rounded-xl transition-all active:scale-95"
            >
              Free Quote लें
            </a>
          </div>
