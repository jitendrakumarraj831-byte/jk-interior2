            ...
            >
              {visibleImages.map((img, index) => (
                <motion.button
                  ...
                  variants={fadeSlideUpItem}
                  onClick={() => openLightbox(index)}
                  className="group relative w-full block break-inside-avoid mb-4 overflow-hidden rounded-xl glass-panel border border-gold/15 shadow-lg transition-all duration-300 hover:border-gold/40 hover:shadow-xl hover:shadow-gold/10"
                  aria-label={`View ${img.alt}`}
                >
                  <div className={`relative w-full ${aspectVariants[index % aspectVariants.length]}`}>
                    <Image
                      src={img.src}
                      alt={img.alt}
                      fill
