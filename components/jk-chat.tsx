...
import {
  type Message, type ConvMsg, type Lead, type LeadCard, type Role, type MsgKind,
  WA_NUMBER,
} from "@/components/chat/chat-types"
import { TypingDots } from "@/components/chat/typing-dots"
import { MessageBubble } from "@/components/chat/message-bubble"
import { IClose, ISend, ISparkle, IMic, IMicStop } from "@/components/chat/chat-icons"
...
// Icons, sub-components, and types are imported from @/components/chat/*
...
export default function JKChat() {
  ...
  const { isListening, voiceSupported, toggleVoice, stopListening } = useVoiceRecognition(setInputCb)
...
const tLower = text.toLowerCase()
  ...
  return
    ...
    setTyping(false)
    sendLock.current = false
  }, [input, lead, typing, lastTopic, roomSize, aiMode, collectStep, pendingEstimate, stopListening])
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            >
              {messages.map((m, idx) => (
                <MessageBubble key={m.id} m={m as Message & { galleryType?: string }} idx={idx} />
              ))}
              {typing && <TypingDots />}
            </div>
