            ...
            <div className="shrink-0 flex items-center gap-2 border-t border-gray-200 bg-white px-3 md:px-4 py-2 md:py-3 pb-[max(10px,env(safe-area-inset-bottom))]">
              ...
              {voiceSupported && (
                <motion.button
                  onClick={toggleVoice}
                  whileTap={{ scale: 0.9 }}
                  className={`relative flex h-9 w-9 md:h-10 md:w-10 shrink-0 items-center justify-center rounded-full transition-all shadow ${
                    isListening
              ...
              <input
                ...
                className="flex-1 rounded-full border border-gray-200 bg-gray-50 px-3 md:px-4 py-2 md:py-2.5 text-[12px] md:text-[13px] text-gray-800 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-colors"
                autoComplete="off"
              />
              <button
                onClick={() => send()}
                disabled={!input.trim() || typing}
                className="flex h-9 w-9 md:h-10 md:w-10 shrink-0 items-center justify-center rounded-full bg-emerald-700 text-white shadow hover:bg-emerald-600 active:scale-90 transition-all disabled:opacity-40"
              >
                <ISend />
              </button>
            </div>
